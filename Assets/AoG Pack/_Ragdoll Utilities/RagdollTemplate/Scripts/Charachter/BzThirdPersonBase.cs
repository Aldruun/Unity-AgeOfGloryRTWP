...
namespace BzKovSoft.RagdollTemplate.Scripts.Charachter
{
	...
	public abstract class BzThirdPersonBase : MonoBehaviour, IBzRagdollCharacter, IBzThirdPerson
	{
		...
		// constants:
		private const float JumpPower = 5f;		// determines the jump force applied when jumping (and therefore the jump height)
		private const float AirSpeed = 5f;		// determines the max speed of the character while airborne
		private const float AirControl = 2f;	// determines the response speed of controlling the character while airborne
		private const float StationaryTurnSpeed = 180f;	// additional turn speed added when the player is stationary (added to animation root rotation)
		private const float MovingTurnSpeed = 360f;		// additional turn speed added when the player is moving (added to animation root rotation)
		private const float RunCycleLegOffset = 0.2f;	// animation cycle offset (0-1) used for determining correct leg to jump off

		protected Animator _animator; // The animator for the character

		// parameters needed to control character
		private bool _onGround; // Is the character on the ground
