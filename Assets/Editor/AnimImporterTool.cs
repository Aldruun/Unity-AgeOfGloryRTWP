...
public class AnimImporterTool : EditorWindow
{
    ...
    private class Settings
    {
        ...
        public bool root_transform_y;
        public bool root_transform_y_bakeintopose;
        public bool root_transform_y_keep_original;
        public float root_transform_y_offset;
        public bool root_transform_xz;
        public bool root_transform_xz_bakeintopose;
        public bool root_transform_xz_keep_original;
        internal string loop_keywords;
    }
    [MenuItem("AoG Utilities/Animation/AnimClip Batch Settings")]
    ...
    private void OnGUI()
    {
        ...
        {
            settings.rename_prefix = EditorGUILayout.TextField("Prefix", settings.rename_prefix);
        ...
        {
            settings.loop_keywords = EditorGUILayout.TextField("Keywords", settings.loop_keywords);
        ...
        EditorGUILayout.EndToggleGroup();
        GUILayout.Space(5);
        settings.root_transform_y = EditorGUILayout.BeginToggleGroup("Root transform Y", settings.root_transform_y);
        {
            settings.root_transform_y_bakeintopose = EditorGUILayout.Toggle("Bake into pose", settings.root_transform_y_bakeintopose);
            settings.root_transform_y_keep_original = EditorGUILayout.Toggle("Keep original", settings.root_transform_y_keep_original);
            settings.root_transform_y_offset = EditorGUILayout.FloatField("Offset", settings.root_transform_y_offset);
        }
        EditorGUILayout.EndToggleGroup();
        GUILayout.Space(5);
        settings.root_transform_xz = EditorGUILayout.BeginToggleGroup("Root transform XZ", settings.root_transform_xz);
        {
            settings.root_transform_xz_bakeintopose = EditorGUILayout.Toggle("Bake into pose", settings.root_transform_xz_bakeintopose);
            settings.root_transform_xz_keep_original = EditorGUILayout.Toggle("Keep original", settings.root_transform_xz_keep_original);
        }
        EditorGUILayout.EndToggleGroup();
        GUILayout.Space(5);
    ...
    public void process_dir()
    {
        ...
        if (allFiles.Count > 0)
        {
            ...
            {
                int idx = allFiles.IndexOf("Assets");
    ...
    private void RenameAndImport(ModelImporter asset, string name)
    {
        ...
        if (settings.rename_anim_clips_tolower)
            name = name.ToLower();
        if(!string.IsNullOrEmpty(settings.rename_prefix))
            name = settings.rename_prefix + name;
        for (int i = 0; i < clipAnimations.Length; i++)
        {
            ...
            //{
            //    string[] keywords = settings.loop_keywords.Split(new char[] { ' ', ',', ';'}, System.StringSplitOptions.RemoveEmptyEntries);
            //    bool loop = keywords.Length == 0 || keywords.Any(k => name.Contains(k));
            //    clip.loopTime = loop && settings.loop_anim_clips_time;
            //    clip.loopPose = loop && settings.loop_anim_clips_pose;
            //}
            ...
            foreach(ClipAnimationInfoCurve curveArray in clip.curves)
            {
                ClipAnimationInfoCurve curves = curveArray;
            ...
            if (settings.root_transform_y)
            {
                clip.lockRootHeightY = settings.root_transform_y_bakeintopose;
                clip.keepOriginalPositionY = settings.root_transform_y_keep_original;
                clip.heightOffset = settings.root_transform_y_offset;
            }
            if (settings.root_transform_xz)
            {
                clip.lockRootPositionXZ = settings.root_transform_xz_bakeintopose;
                clip.keepOriginalPositionXZ = settings.root_transform_xz_keep_original;
            }
        }
