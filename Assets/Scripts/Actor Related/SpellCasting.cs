...
public static class SpellCasting
{
    ...
    /// <returns></returns>
    public static bool InflictMagicEffectsAtActor(Spell spell, Actor target)
    {
        ...
        ApplyVFXToActor(target, spell.magicEffectsData[0], 0.5f);
        foreach(EffectData eff in spell.magicEffectsData)
        {
            target.ApplyStatusEffect(eff.effect, eff.duration);
        }
    ...
    public static void CastSpraySpellInDirection(Spell spell, Actor caster, Vector3 targetPoint, float duration)
    {
        ...
        caster.ActorStats.ModifyStatBase(ActorStat.MAXHITPOINTS, -spell.Cost, ModType.ADDITIVE);
    }

    public static void LaunchMagicProjectile(Spell spell, Actor caster, Actor target)
    {
        if(caster == null || caster.dead)
        {
            return;
        ...
        //ActorUtility.ModifyActorMagicka(caster.ActorRecord, -spell.cost, ModType.ADDITIVE);
        projectile.Launch(caster, caster.Equipment.spellAnchor.position, target, spell.deliveryType, ProjectileType.Missile, spell.magicEffectsData[0].damageType, spell.magicEffectsData[0].magnitude, effectData.aoeRadius, spell.magicEffectsData[0].effectDiameter, spell.activationRange);
    }
    ...
    private static void ApplyVFXToActor(Actor target, EffectData effectData, float delay)
    {
        GameObject vfxObj = PoolSystem.GetPoolObject(effectData.vfxid_projectile, ObjectPoolingCategory.VFX);
        //Debug.Log(spellAnchor.name);
        VFXPlayer.TriggerVFX(vfxObj, target.transform, Vector3.zero, Quaternion.identity, false, effectData.duration);
    }
    ...
    public static Spell ChooseBestSpell(Actor caster, Actor target)
    {
        ...
        for(int i = 0; i < caster.Spellbook.SpellData.Count; i++)
        {
            Spell currSpell = caster.Spellbook.SpellData[i].spell;
        ...
        {
            if(caster.debugAnimation)
                Debug.Log(caster.GetName() + ": Chose " + highestPrioritySpell.spellTargetType.ToString() + " '<color=white>" + highestPrioritySpell.Name + "</color>'\n" +
